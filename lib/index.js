...
function processConfig(config, src) {
  // If the configuration file is a string, assume the dictionary has been installed in node_modules
  if (typeof config === "string") {
    config = {
      dicFile: `./node_modules/${config}/index.dic`,
      affFile: `./node_modules/${config}/index.aff`
    };
  }
  config = config || {};
...
function spellcheck(config) {
  ...
  return function (files, metalsmith, done) {
    ...
    if (config.dicFile.indexOf("node_modules") !== -1) {
      files[config.dicFile] = {
        contents: fs.readFileSync(config.dicFile)
      }
      files[config.affFile] = {
        contents: fs.readFileSync(config.affFile)
      }
    }
