...
function exceptionToPatterns(pattern, files) {
  var parts = pattern.match(/^\/(.*?)\/([gim]*)$/);
  if (parts) {
    var newPattern = parts[1];
    var flags = parts[2];
    if (newPattern.lastIndexOf("\\b", 0) === -1) {
      newPattern = "\\b" + newPattern;
    }
    if (newPattern.indexOf("\\b", (newPattern.length - 2)) === -1) {
      newPattern = newPattern + "\\b";
    }
    ...
    return _.map(cleanText(pattern), function (word) {
      return {'word': word, 'files': files, 'pattern': new RegExp("\\b" + word + "\\b", "g") }
    });
  }
};
            ...
            if (!(pattern_exceptions[info.word])) {
              ...
              if (info.files === true) {
                ...
                pattern_exceptions[info.word].files = 
                  _.union(info.files, pattern_exception[info.word].files);
              }
            }
          };
          ...
          async.forEachOfLimit(htmlfiles, 8, function(file, filename, finished) {
            ...
            $('body *').contents().filter(function (index, element) {
              ...
              _.each(pattern_exceptions, function(pattern) {
                if (pattern.files === true || (pattern.files.indexOf(filename) !== -1)) {
