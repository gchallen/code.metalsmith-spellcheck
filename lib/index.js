          ...
          _.each(metadata_exception_phrases, function (phrase) {
            _.each(cleanText(phrase), function (word) {
              pattern_exceptions[word] = {
                'files': true,
                'pattern': wordToRegex(word)
              };
            });
          });
          _.each(metadata_exception_patterns, function (pattern) {
            pattern_exceptions[pattern] = {
              'files': true,
              'pattern': wordToRegex(pattern)
            };
          });
          ...
          async.filter(uniqwords, function (word, innerCallback) {
            innerCallback(!(_.some(pattern_exceptions, function (item) {
              return (item.pattern.exec(word) != null &&
                      (item.files === true ||
                       (item.files.length == wordstofilenames[word].length &&
                        _.intersection(item.files, wordstofilenames[word]).length == item.files.length)));
            })));
          }, function (results) {
            uniqwords = results;
            callback();
